...
* The size and positioning of the isometric coordinate system is controlled by 
  arguments `isocubesGrob(coords, max_y, xo, yo)`
...
single_fuck <- bdftools::bdf_create_df(bdf, "fuck")

N    <- 10
cols <- rainbow(N)

multiple_fucks <- purrr::map_dfr(seq(N), function(i) {
  single_fuck$z <- i
  single_fuck$col <- cols[i]
  single_fuck
}) 

cubes  <- isocubesGrob(multiple_fucks, max_y = 120, fill = multiple_fucks$col)
grid.newpage(); grid.draw(cubes)
