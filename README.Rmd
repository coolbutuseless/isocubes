...
```{r coords}
x <- c(1, 1,   3, 3, 3, 3,   5, 5, 5, 5, 5, 5)
y <- c(1, 1,   1, 1, 2, 2,   1, 1, 2, 2, 3, 3)

z <- rep(c(1, 2, 3), each = length(x))
x <- rep(x, 3)
y <- rep(y, 3)

fill <- c(rep('red', 2), rep('green', 4), rep('blue', 6))
fill <- rep(fill, 3)

coords <- data.frame(x = x, y = y, z = z)
cubes  <- isocubesGrob(coords, fill = fill, xyplane = 'left', handedness = 'right', size = 2)
...
cubes  <- isocubesGrob(coords, xyplane = 'right', handedness = 'left', size = 3)
grid.newpage(); grid.draw(cubes)
```
