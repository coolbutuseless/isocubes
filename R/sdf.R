...
coords_sphere <- function(x, y, z, r) {
  ...
  inside <- with(coords, ((x - xc)^2 + (y - yc)^2 + (z - zc)^2) < r*r)
...
#' @export
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
coords_heightmap <- function(mat, col = NULL, scale = 1, solid = TRUE, verbose = FALSE) {
  ...
  coords <- data.frame(
    ...
    z = rep(seq_len(ncol(mat)), each = nrow(mat)),
    y = round(as.vector(mat) * scale)
  )
  
  coords$col <- as.vector(col)
  
  # Extrude the cubes down 
  if (solid) {
    start <- nrow(coords)
...
if (FALSE) {
  ...
  cubes <- isocubesGrob(coords, max_y = 45, fill = 'red', fill_left = 'white', fill_right = 'blue')
  grid.newpage()
  grid.draw(cubes)
